...
編集リズムは既存JSONの `shot` 区間から「ショット長の推移」と「30秒ごとの切替回数」を描画します。最終区間の実秒数を示し、動画開始は切替に数えません。グラフを押すと該当時刻・章名を表示し、元動画を選択済みなら再生位置も移動します。速度を品質スコアに変換せず、拍・発話との同期や編集の目的への適合は未判定と明示します。音声解析が60秒だけでも、ショットデータが全編分あれば編集グラフは全編を表示します。
